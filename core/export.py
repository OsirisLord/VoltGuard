...
from config.constants import IEC_DISCLAIMER, VOLTAGE_DROP_LIMIT
...
class PDFReport(FPDF):
    ...
    def header(self):
        ...
        self.cell(0, 10, 'VoltGuard Calculation Report', align='C', new_x='LMARGIN', new_y='NEXT')
        self.set_font('Helvetica', 'I', 10)
        self.cell(0, 6, 'IEC 60364-5-52 Compliant', align='C', new_x='LMARGIN', new_y='NEXT')
        self.ln(5)
...
def export_to_pdf(report: CalculationReport, filepath: str, engineer_name: Optional[str] = None, approver_name: Optional[str] = None) -> None:
    ...
    if report.recommended:
        ...
        pdf.cell(0, 8, f'Recommended Cable Size: {rec.size} mm2', fill=True, new_x='LMARGIN', new_y='NEXT')
        pdf.cell(0, 6, f'Effective Ampacity: {rec.effective_ampacity:.1f} A', new_x='LMARGIN', new_y='NEXT')
        pdf.cell(0, 6, f'Voltage Drop: {rec.voltage_drop_percent:.2f}% (Limit: {VOLTAGE_DROP_LIMIT}%)', new_x='LMARGIN', new_y='NEXT')
        pdf.cell(0, 6, f'Suggested MCB Rating: {rec.mcb_rating} A', new_x='LMARGIN', new_y='NEXT')
        pdf.cell(0, 6, f'Earth Fault Loop Impedance (Zs): {rec.earth_fault_loop:.3f} Ohms', new_x='LMARGIN', new_y='NEXT')
        pdf.cell(0, 6, f'Estimated Cost: ${rec.cost_estimate:.2f}', new_x='LMARGIN', new_y='NEXT')
    else:
        pdf.set_fill_color(255, 200, 200)
        pdf.cell(0, 8, 'No suitable cable size found. Consider larger cables or parallel runs.', fill=True, new_x='LMARGIN', new_y='NEXT')
    ...
    headers = ['Size', 'Ampacity', 'VD %', 'SC Min', 'MCB', 'Zs', 'Cost', 'Amp', 'VD', 'SC', 'Status']
